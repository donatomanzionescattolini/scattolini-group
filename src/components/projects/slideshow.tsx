...
const SlideshowGallery = (props: PropsSlideshow) => {
    ...
    const [returnEls] = useState(() => {
        ...
        areaObjects.forEach((areaObject: Area) => {
            ...
            areaDesarrollos.forEach((x: Project) => {
                temp.push(
                    <Carousel.Item
                        key={index}
                        id={`second-carousel-${index}`}
                        style={{
                            backgroundImage: `url('https://pagina-mama.s3.amazonaws.com/assets2/areas/${areaObject.nameForProjectFolders}/${x.nameForProjectFolders}.webp`,
                        }}
                    >
                        <Carousel.Caption>
